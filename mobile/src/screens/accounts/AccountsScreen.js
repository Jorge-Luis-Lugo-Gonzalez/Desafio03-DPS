...
export default function AccountsScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTarget) {
        await client.put(`/accounts/${editTarget.id}`, { name: name.trim() });
      } else {
        await client.post('/accounts', { name: name.trim() });
      }
    ...
    <View style={styles.container}>
      ...
      {loading
        ...
        : <FlatList
            data={accounts}
            keyExtractor={a => a.id}
            ListEmptyComponent={<Text style={styles.empty}>No tienes cuentas aún</Text>}
            renderItem={({ item }) => (
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{editTarget ? 'Editar cuenta' : 'Nueva cuenta'}</Text>
            <TextInput
              style={styles.input}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', margin: 8, marginHorizontal: 12, borderRadius: 10, padding: 16, flexDirection: 'row', justifyContent: 'space-between', elevation: 1 },
  accountName: { fontSize: 16, fontWeight: 'bold' },
  balanceLabel: { color: '#aaa', fontSize: 12, marginTop: 4 },
